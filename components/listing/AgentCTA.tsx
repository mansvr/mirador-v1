...
interface AgentCTAProps {
  ...
  sceneTitle: string;
  /** Inside tour page listing card (no full-bleed strip chrome). */
  variant?: "strip" | "card";
}

export function AgentCTA({ listing, sceneTitle, variant = "strip" }: AgentCTAProps) {
  ...
  const wrapClass =
    variant === "card"
      ...
      : "border-t border-mirador-border px-4 py-4 md:px-6 bg-mirador-surface";
  ...
  return (
    <div className={wrapClass}>
      <a
        ...
        className="flex w-full min-w-0 items-center justify-center gap-2 rounded-xl bg-[#25D366] px-3 py-3 text-center text-sm font-semibold leading-snug text-white transition-opacity hover:opacity-90 [overflow-wrap:anywhere] sm:px-4"
      >
